...
"""
def nbMonths(oldVal, newVal, savingperMonth, percentLossByMonth):
    months, savings = 0, 0
    percentLossByMonth /= 100.0
    
    while oldVal + savings < newVal:
        if months % 2 != 0: percentLossByMonth += .005
        oldVal -= oldVal * percentLossByMonth; newVal -= newVal * percentLossByMonth
        months += 1
        savings += savingperMonth

    return [months, round(oldVal + savings - newVal)]

### TESTS ### 

#res = nbMonths(12000, 8000, 1000, 1.5)
res = nbMonths(2000, 8000, 1000, 1.5)
print(res)
