...
def phone(numbers_string, num):
    phone_dict = {}
    ADDRESS_PATTERN = re.compile(r"[/+/*;$!?,:]")
    SPACE_PATTERN = re.compile("\s{2,}")

    for string in numbers_string.split('\n'):
        try:
            name = re.search('<.*>', string).group()[1:-1]
            number = re.search('\+\d{1,2}-\d{3}-\d{3}-\d{4}', string).group()[1:] # +X-abc-def-ghij X(1 or 2 digits)

            # Strip name and number out of string, then replace all bogus characters
            address = re.sub(ADDRESS_PATTERN, "", string.replace('<' + name + '>', '').replace(number, '').replace('_', ' ')).strip()
            address = re.sub(SPACE_PATTERN, " ", address)

            if number in phone_dict:
                phone_dict[number] = "Error => Too many people: " + number
            else:
                phone_dict[number] = ("Phone => %s, Name => %s, Address => %s" % (number, name, address))
        except:
            pass

    return phone_dict[num] if num in phone_dict else ("Error => Not found: " + num)
