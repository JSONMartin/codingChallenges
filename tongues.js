...
function tongues(code) {
  let res = "", idx = 0, letterIdx;

  const matchCase = (letter) => (code[idx] === letter);

  const vowels = ['a', 'i', 'y', 'e', 'o', 'u'],
        consonants = ['b', 'k', 'x', 'z', 'n', 'h', 'd', 'c', 'w', 'g', 'p', 'v', 'j', 'q', 't', 's', 'r', 'l', 'm', 'f'];

  for (let letter of code.toLowerCase()) {
      if ((letterIdx = vowels.indexOf(letter)) > -1) {
        res += matchCase(letter, letterIdx) ? vowels[(letterIdx + 3) % vowels.length] : vowels[(letterIdx + 3) % vowels.length].toUpperCase();
      }
      else if ((letterIdx = consonants.indexOf(letter)) > -1){
        res += matchCase(letter, letterIdx) ? consonants[(letterIdx + 10) % consonants.length] : consonants[(letterIdx + 10) % consonants.length].toUpperCase();
      }
      else {
        res += letter;
      }
...
// TESTS
let res = tongues('Ita dotf ni dyca nsaw ecc.') //=> 'One ring to rule them all.'
console.log(res)
