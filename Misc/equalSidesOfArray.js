...
function findEvenIndexWithReduce(arr) { // O(n^3)
    for (let i = 1; i < arr.length - 1; i++) {
        let leftSlice = arr.slice(0,i), rightSlice = arr.slice(i + 1);

        let left = leftSlice.length > 0 ? leftSlice.reduce((prev, cur) => prev + cur) : 0;
        let right = rightSlice.length > 0 ? rightSlice.reduce((prev, cur) => prev + cur) : 0;
...
// Tests
let res = findEvenIndex([1,2,3,4,3,2,1]); // => ,3, "The array was: [1,2,3,4,3,2,1] \n");
